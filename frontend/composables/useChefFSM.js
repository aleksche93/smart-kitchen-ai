...
export function useChefFSM() {
  const updateState = (apiResponse) => {
    if (apiResponse && apiResponse.chef_response) {
      ...
      // Normalize emotion state immediately to ensure a strict FSM
      chefState.emotionDisplay = chefResp.emotion_displayed?.toUpperCase() || 'IDLE'
      ...
      if (chefState.emotionDisplay === 'ANGRY' || chefState.emotionDisplay === 'CHAOTIC') {
