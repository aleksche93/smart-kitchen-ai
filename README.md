...
- **Cognitive Glassmorphism & Sarcasm**: A floating `ThoughtTicker` HUD natively integrates with the background, actively broadcasting the core backend processes and flexing dynamically into empty chat spaces. A Pinia-driven **Sarcastic Idle Engine** ensures the Chef produces witty observations when left inactive.
